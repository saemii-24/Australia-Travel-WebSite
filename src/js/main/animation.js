...
function imageMove(elTop, elHeight, el, ifPCsize, ifOthersize) {
  if (windowSize <= 1200) {
    let move = (scrollHeight - elTop) / 60;
    if (move < elHeight) {
      //빈공간이 생기지 않도록 -50을 더해준다
      el.style.backgroundPosition = `center ${-50 - ifPCsize + move}px`;
    } else if (move > elHeight) {
      el.style.backgroundPosition = `center ${-50 - ifPCsize + move}px`;
    }
  } else {
    let move = (scrollHeight - elTop) / 70;
    if (move < elHeight) {
      el.style.backgroundPosition = `center ${-50 - ifOthersize + move}px`;
    } else if (move > elHeight) {
      el.style.backgroundPosition = `center ${-50 - ifOthersize + move}px`;
    }
  }
